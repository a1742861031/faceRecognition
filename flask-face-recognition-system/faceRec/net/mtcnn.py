...
class mtcnn():
    ...
    def detectFace(self, img, threshold):
        ...
        #   每一次缩放的比例
        # -----------------------------#
        scales = utils.utils.calculateScales(img)
        ...
        for i in range(len(scales)):
            ...
            #   解码的过程
            # -------------------------------------#
            rectangle = utils.utils.detect_face_12net(cls_prob, roi, out_side, 1 / scales[i], origin_w, origin_h,
                                                threshold[0])
        ...
        #   进行非极大抑制
        # -------------------------------------#
        rectangles = np.array(utils.utils.NMS(rectangles, 0.7))
        ...
        #   解码的过程
        # -------------------------------------#
        rectangles = utils.utils.filter_face_24net(cls_prob, roi_prob, rectangles, origin_w, origin_h, threshold[1])
        ...
        #   解码的过程
        # -------------------------------------#
        rectangles = utils.utils.filter_face_48net(cls_prob, roi_prob, pts_prob, rectangles, origin_w, origin_h, threshold[2])
