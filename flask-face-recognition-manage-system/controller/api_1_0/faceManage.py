...
def getAllFace():
  ...
  path：L:\毕设项目\keras-face-recognition-master\face_dataset
    ...
    if len(request.args) == 0:
        ...
        faces = models.Face.query.filter(
            models.Face.name.like(("%" + request.args["name"] + "%")) if request.args["name"] is not None else "").all()
    data = []
    for face in faces:
        try:
            f = open(r'L:\毕设项目\keras-face-recognition-master\face_dataset\\' + face.name + '.jpg', 'rb')  # 二进制方式打开图文件
        except:
            f = open(r'L:\毕设项目\keras-face-recognition-master\images\\' + '默认' + '.jpg', 'rb')  # 二进制方式打开图文件
...
def addFace():
    ...
    face = models.Face(state=state, reg_time=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), name=name)
    image_base64 = str(req_dict.get("imageSrc"))
    if image_base64 != '':
        imagedata = base64.b64decode(image_base64.split(',')[1])
        file = open(r'L:\毕设项目\keras-face-recognition-master\face_dataset\\' + req_dict.get("name") + '.jpg',
                    "wb")
    ...
    db.session.add(face)
    db.session.commit()
    return jsonify(code=201, msg="新增用户成功")
